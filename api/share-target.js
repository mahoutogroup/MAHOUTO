export default async function handler(req, res) {
  // CAS 1: Android envoie en POST
  if (req.method === 'POST') {
    return res.redirect(307, '/api/share-target');
  }

  // CAS 2: Afficher la page
...
.icon{font-size:28px}
button{width:100%;padding:16px;background:#FFC107;color:#000;border:0;border-radius:12px;font-weight:bold;font-size:16px}
...
<div onclick="select(this)" data-dest="serigraphes" class="item">LES SÉRIGRAPHES</div>
...
function select(el){
  document.querySelectorAll('.item').forEach(i=>i.style.border='2px solid transparent');
  el.style.border='2px solid #FFC107';
  destination = el.dataset.dest; // CORRIGÉ
}

// LA MAGIE : Récupérer les fichiers
if ('launchQueue' in window) {
  launchQueue.setConsumer(async launchParams => {
    if (launchParams.files) {
      fichiers = await Promise.all(launchParams.files.map(f => f.getFile())); // CORRIGÉ
      if(launchParams.targetURL){
        const url = new URL(launchParams.targetURL);
        document.getElementById('text').value = url.searchParams.get('text') || '';
      }
      afficher();
    }
  });
